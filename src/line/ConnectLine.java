...
public class ConnectLine {
    ...
    public static final int LINE_TYPE_RECT_2BREAK_EXTRA_L = 3;
    public static final int LINE_TYPE_RECT_2BREAK_EXTRA_R = 4;
    ...
    public void paint(Graphics2D g2d) {
        switch (lineType) {
            ...
            case LINE_TYPE_RECT_2BREAK:
                paint2Breaks(g2d);
                break;
            case LINE_TYPE_RECT_2BREAK_EXTRA_L:
                paint2BreaksExtraL(g2d);
                break;
            case LINE_TYPE_RECT_2BREAK_EXTRA_R:
                paint2BreaksExtraR(g2d);
                break;
        }
    }

    private void paint2BreaksExtraR(Graphics2D g2d) {
    	paint2BreaksExtra(g2d, Math.max(p1.x, p2.x) + 7);
		
	}

	private void paint2BreaksExtraL(Graphics2D g2d) {
		paint2BreaksExtra(g2d, Math.min(p1.x, p2.x) - 7);
	...
	protected void paintSimple(Graphics2D g2d) {
        ...
        switch (lineArrow) {
            case LINE_ARROW_DEST://Hauptnutzung
                paintArrow(g2d, p1, p2); 
                paintRelations(g2d, p1, p2, "1", "n");
                break;
    ...
    private void paintRelations(Graphics2D g2d, Point p1, Point p2, String rel1,
			String rel2) {
		if(p1.x < p2.x){
			g2d.drawString(rel1, p1.x, p1.y-2);
			g2d.drawString(rel2, p2.x-15, p2.y);
		}else{
			g2d.drawString(rel1, p1.x, p1.y);
			g2d.drawString(rel2, p2.x, p2.y);
		}
    ...
    protected void paint2BreaksExtra(Graphics2D g2d, int extrem) {
    	g2d.drawLine(p1.x, p1.y, extrem , p1.y);
        g2d.drawLine(extrem, p1.y, extrem , p2.y);
        g2d.drawLine(extrem, p2.y, p2.x, p2.y);
        
        switch (lineArrow) {
            case LINE_ARROW_DEST:
                paintArrow(g2d, new Point(extrem, p2.y), p2);
                paintRelations(g2d, p1, p2, "1", "n");
                break;
            case LINE_ARROW_SOURCE:
                paintArrow(g2d, new Point(extrem, p1.y), p1);
                paintRelations(g2d, p1, p2, "1", "n");
                break;
            case LINE_ARROW_BOTH:
                paintArrow(g2d, new Point(extrem, p2.y), p2);
